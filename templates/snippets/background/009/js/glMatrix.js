...
vec3.distance = function(vec1,vec2){
        var x = vec1[0]-vec2[0], y = vec1[1]-vec2[1], z = vec1[2]-vec2[2];
        return Math.sqrt(x*x + y*y + z*z);
};
vec3.multiply = function(vec,m,dest){
        if(!dest || vec == dest) {
                vec[0] *= m;
                vec[1] *= m;
                vec[2] *= m;
...
vec3.divsion = function(vec,d,dest){
        if(!dest || vec == dest) {
                vec[0] /= d;
                vec[1] /= d;
                vec[2] /= d;
 ...
 * dest if specified, mat otherwise
 */
mat4.mirror = function(mat, axis, dest) {
        ...
        var b00 = 1-2*x*x, b01 = -2*x*y, b02 = -2*x*z;
        var b10 = -2*x*y, b11 = 1-2*y*y, b12 = -2*y*z;
        var b20 = -2*x*z, b21 = -2*y*z, b22 = 1-2*z;
